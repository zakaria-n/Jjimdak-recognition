...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    let logo: UIImageView = {
        let extractedExpr = #imageLiteral(resourceName: "couscous")
        let image = UIImageView(image: extractedExpr.resized(newSize: CGSize(width: screenWidth - 40, height: (screenWidth - 40)/1.6 )))
        image.translatesAutoresizingMaskIntoConstraints = false
        return image
    }()
    
    let upload: largeFilledButton = {
    ...
    let openCamera: largeFilledButton = {
        let button = largeFilledButton()
        button.translatesAutoresizingMaskIntoConstraints = false
        ...
        if let pickedImage = info[UIImagePickerController.InfoKey.originalImage] as? UIImage {
            ...
            let request = VNCoreMLRequest(model: model) {[weak self] request, error in
                ...
                DispatchQueue.main.async {[weak self] in
                    let controller = JjimdakImageViewController()
                    controller.JjimdakImage.image = pickedImage
                    if topResult.identifier == "Jjimdak" {
                        controller.isOrNotJjimdak.image = #imageLiteral(resourceName: "isCouscous")
                        self?.present(controller, animated: true)
                    } else {
                        controller.isOrNotJjimdak.image = #imageLiteral(resourceName: "isNotCouscous")
                        self?.present(controller, animated: true)
                    }
                }
            }
    ...
    @objc func buttonToUpload(_ sender: largeFilledButton) {
    ...
    @objc func buttonToOpenCamera(_ sender: largeFilledButton) {
